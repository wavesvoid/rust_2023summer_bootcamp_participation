...
fn test3_purchase_failures() {
    ...
    vmachine.insert_coins(vec![Coin::One]);

    let purchase_4_succ = vmachine.purchase(product3_name, 1);

    let (mut vmachine, rest1) = vmachine.finish();
    let mut vmachine = vmachine.restart().unwrap();

    let purchase_5_fail = vmachine.purchase(product3_name, 1);
    ...
    assert_eq!(
        purchase_4_succ.unwrap().products,
        vec![Product("Doomed".into())]
    );
    assert_eq!(purchase_5_fail.unwrap_err(), VMError::NotEnoughCoinsGiven);

    assert_eq!(rest1, vec![Coin::Fifty]);
}

#[test]
fn test_serial_purchase() {
    // 1 of every coin
    ...
    vmachine.insert_coins(vec![Coin::Ten]);

    let purchase_71_succ = vmachine.purchase("Doomed", 2);
    let purchase_72_succ = vmachine.purchase("Doomed", 1);
    let purchase_73_succ = vmachine.purchase("Doomed", 1);
    let purchase_74_succ = vmachine.purchase("Doomed", 1);
    let purchase_75_succ = vmachine.purchase("Doomed", 2);
    let purchase_76_succ = vmachine.purchase("Doomed", 3);
    let purchase_77_fail = vmachine.purchase("Doomed", 3);
    ...
    assert_eq!(
        ...
        (0..2)
            .into_iter()
            .map(|_| Product("Doomed".into()))
            .collect::<Vec<_>>()
    );
    assert_eq!(
        purchase_72_succ.unwrap().products,
        vec![Product("Doomed".into())]
    );
    assert_eq!(
        purchase_73_succ.unwrap().products,
        vec![Product("Doomed".into())]
    );
    assert_eq!(
        purchase_74_succ.unwrap().products,
        vec![Product("Doomed".into())]
    );
    assert_eq!(
        purchase_75_succ.unwrap().products,
        (0..2)
            .into_iter()
            .map(|_| Product("Doomed".into()))
            .collect::<Vec<_>>()
    ...
    assert_eq!(
        purchase_76_succ.unwrap().products,
        (0..3)
            .into_iter()
            .map(|_| Product("Doomed".into()))
            .collect::<Vec<_>>()
    ...
    assert_eq!(purchase_77_fail.unwrap_err(), VMError::NotEnoughItemsInSlot);
...
#[test]
fn test6_not_enough_to_buy() {
    let mut vmachine = VMBuilder::new()
        .set_coins_per_stack(0)
        .add_products(vec![
            // name , amount, price
            ProductSlot::new("Chocolate", 4, 1).unwrap(),
        ])
        .build()
        .begin();
    ...
    let purchase_1_succ = vmachine.purchase("Chocolate", 1);
    let purchase_2_succ = vmachine.purchase("Chocolate", 1);
    ...
    let purchase_3_succ = vmachine.purchase("Chocolate", 1);
    let purchase_3_fail = vmachine.purchase("Chocolate", 1);

    assert_eq!(
        purchase_1_succ.unwrap().products,
        vec![Product("Chocolate".into())]
    );
    assert_eq!(
        purchase_2_succ.unwrap().products,
        vec![Product("Chocolate".into())]
    );
    assert_eq!(
    ...
    assert_eq!(purchase_3_fail.unwrap_err(), VMError::NotEnoughCoinsGiven);
}
