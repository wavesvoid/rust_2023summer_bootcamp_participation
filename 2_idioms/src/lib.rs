...
impl VendingMachine<vm_states::SessionActive> {
    ...
    pub fn purchase<PN: Into<units::ProductName>>(
        ...
        for coin in units::Coin::make_iter().rev() {
            ...
            let coin_size: &usize = &coin.into();
            let available = *self.coin_stack.get(&coin).unwrap();
            let mut fits_count = balance_rest / coin_size;

            if fits_count != 0 && available > 0 {
                if fits_count > available {
                    fits_count = available
                }

                balance_rest -= coin_size * fits_count;
                (0..fits_count).for_each(|_| change_to_give.push(coin));
            }
        }
