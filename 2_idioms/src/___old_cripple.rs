...
pub struct VendingMachine<S> {
    ...
    consumed_coins: HashMap<units::Coin, usize>,
...
impl<S> VendingMachine<S> {
    fn compose_from_state<T>(&mut self) -> VendingMachine<T> {
        VendingMachine::<T> {
            ...
            consumed_coins: std::mem::take(&mut self.consumed_coins),
            _state: PhantomData,
        }
    }
...
impl VendingMachine<vm_states::SessionActive> {
    ...
    pub fn purchase<PN: Into<units::ProductName>>(
        ...
        println!("PUr {:?}", self.coin_stack);
        let product_name = product_name.into();
        let slot = self
            .slots.get_mut(&product_name).ok_or(VMError::SlotEmpty)?;
        ...
        // Current price number to check the condition
        // of capability to satisfy this purchase with amount of available
        // coins
        let mut price = complete_price;

        // Current balance during calculations
        let mut balance_rest = self.balance - complete_price;

        // Holds a coins that in case of successful purchase
        // will be released to the user when he `finishes` work on the VM
        let mut change_to_give = vec![];

        // Will contain counts for each coin that is consumed from stack
        // (coins iserted are put to the stack immediately after insert)
        // if all conditions met, and product is considered bought
        // this amount of coins is considered as 'spent' on this product
        // and will be considered in the next purchase iteration
        // let mut consumed_coins = vec![];

        // Calculate possibility to give a change
        // along with the rest to give to the user when he finishes work
        for coin in units::Coin::make_iter().rev() {
            // Amount of coins with current nominal
            // that fit into current balance
            let contains_count: usize = balance_rest / &coin.into();

            // If current balance is still unsatisfied
            // and there are some coins with given nominal that fit into balance
            if contains_count != 0 && balance_rest != 0 {
                // Reduce current balance by amount of fitting coins
                balance_rest -= &coin.into() * contains_count;
                // save the coins as the rest to give to the user
                (0..contains_count)
            ...
            let sub = *self.consumed_coins.get(&coin).unwrap_or(&0);
            ...
            // Collect a specific amount of specific coin consumed from stack
            // consumed_coins.push((coin, coin_count));
            self.consumed_coins
                .entry(coin)
        ...
        // Check if user is capable of buying the product
        // and if we are capable of giving the rest (change) to him
        if price > 0 || balance_rest > 0 {
            return Err(VMError::CannotGiveChange);
        }

        // If purchase is successfull record amount of coins
        // that are spent from coin stack
        // self.consumed_coins.clear();
        // consumed_coins.into_iter().map(|(coin, coin_count)| {
        ...
        // Reduce the counter of products in a given slot
        slot.amount -= amount;
        // Remember the amount of rest we owe to the user
        self.change_to_give = change_to_give;

        Ok(Purchase {
            products: (0..amount)
                .map(|_| Product(product_name.clone()))
                .collect(),
        })
    }
    ...
    pub fn finish(&mut self) -> (VendingMachine<vm_states::Finished>, Vec<units::Coin>) {
        ...
        machine.balance = 0;
        std::mem::take(&mut machine.consumed_coins);
